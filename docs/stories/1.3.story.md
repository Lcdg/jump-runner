...
- [x] **Task 1: Create Entity base class** (AC: 1)
  - [x] Créer `src/entities/Entity.ts`
  - [x] Implémenter position, velocity, hitbox, active
  - [x] Méthode abstraite `createHitbox()`
  - [x] Méthode abstraite `update(deltaTime)`
  - [x] Getters: `getHitbox()`, `getPosition()`, `isActive()`

- [x] **Task 2: Create Player entity** (AC: 1, 2)
  - [x] Créer `src/entities/Player.ts` extends Entity
  - [x] Ajouter état: `idle`, `jumping`, `falling`
  - [x] Implémenter `createHitbox()` avec constantes PLAYER
  - [x] Positionner le joueur à gauche (PLAYER.START_X) sur le sol (groundY)
  - [x] Méthode `jump()` pour démarrer le saut
  - [x] Méthode `getState()` pour l'état actuel

- [x] **Task 3: Implement jump physics** (AC: 3, 4, 5)
  - [x] Dans `update(deltaTime)`: appliquer gravité si jumping/falling
  - [x] Appliquer vélocité à la position
  - [x] Détecter atterrissage (position.y >= groundY)
  - [x] Méthode `land()` pour reset vélocité et état
  - [x] Bloquer double saut: `jump()` seulement si state === 'idle'

- [x] **Task 4: Create InputManager** (AC: 3)
  - [x] Créer `src/input/InputManager.ts`
  - [x] Event listeners pour keydown/keyup (Space)
  - [x] Event listeners pour mousedown/mouseup (clic gauche)
  - [x] Callback pour `jump_start` et `jump_end`
  - [x] Prévenir le scroll avec Espace (`preventDefault`)
  - [x] Flag `isJumpKeyDown` pour éviter répétitions

- [x] **Task 5: Integrate Player in Game** (AC: 1, 2)
  - [x] Modifier `Game.ts` pour créer un Player
  - [x] Ajouter InputManager dans Game
  - [x] Connecter InputManager.callback à Game.handleInput
  - [x] Appeler `player.update(deltaTime)` dans Game.update
  - [x] Rendre le joueur dans Game.render

- [x] **Task 6: Render Player and Ground** (AC: 1, 2)
  - [x] Créer méthode `renderPlayer()` dans Game ou Renderer
  - [x] Dessiner rectangle (corps) + cercle (tête) pour le joueur
  - [x] Créer méthode `renderGround()` - ligne horizontale visible
  - [x] Utiliser constantes COLORS pour les couleurs

- [x] **Task 7: Update constants and types**
  - [x] Ajouter constantes PLAYER (START_X, WIDTH, HEIGHT, COLOR, etc.)
  - [x] Ajouter PHYSICS.GRAVITY en pixels/sec² (ex: 1500)
  - [x] Ajouter PHYSICS.JUMP_VELOCITY en pixels/sec (ex: -600, négatif = vers haut)
  - [x] Mettre à jour `types.ts` avec InputAction type

- [x] **Task 8: Write unit tests** (AC: 6)
  - [x] Créer `tests/unit/entities/Player.test.ts`
  - [x] Tester `jump()` change l'état de idle à jumping
  - [x] Tester que le joueur ne peut pas double-sauter
  - [x] Tester la gravité: vélocité augmente avec deltaTime
  - [x] Tester l'atterrissage: reset position et état

- [x] **Task 9: Write E2E test** (AC: 7)
  - [x] Créer `tests/e2e/player-jump.spec.ts`
  - [x] Test: appui sur Espace fait bouger le joueur vers le haut
  - [x] Vérifier que le joueur revient au sol

- [x] **Task 10: Validation finale**
  - [x] `npm run lint` passe
  - [x] `npm run test:run` passe
  - [x] `npm run test:e2e` passe
  - [x] Le joueur s'affiche à gauche sur le sol
  - [x] Appui Espace = saut avec retombée
  - [x] Pas de double saut possible
